...
const raydiumLpPortfolioCommand: CommandHandler = {
  ...
  handler: async (ctx: MyContext, getMessage) => {
    try {
      ...
      // 获取用户的positions信息
      const result = await RaydiumService.getUserPositions(telegramId);
      ...
      console.error('Error in lpPortfolio command:', error);
...
// 处理Take Profit按钮点击
async function handleTakeProfitButton(ctx: MyContext) {
  try {
    const telegramId = ctx.from?.id;
    ...
    const nftMint = callbackData.replace('tp_', '');
    ...
    console.error('Error handling Take Profit button:', error);
  }
}

// 处理Stop Loss按钮点击
async function handleStopLossButton(ctx: MyContext) {
  try {
    const telegramId = ctx.from?.id;
    if (!telegramId) return;
    ...
    const nftMint = callbackData.replace('sl_', '');
    ...
    console.error('Error handling Stop Loss button:', error);
  }
}

// 处理用户输入的Take Profit或Stop Loss值
async function handlePositionSettingInput(ctx: MyContext, getMessage: Function) {
  try {
    const telegramId = ctx.from?.id;
    if (!telegramId) return;
