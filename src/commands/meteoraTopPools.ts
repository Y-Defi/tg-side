...
const meteoraTopPoolsCommand: CommandHandler = {
  command: 'meteora_top_pools',
  description: 'Show top Meteora liquidity pools',
  handler: async (ctx: MyContext, getMessage) => {
    try {
      const lang = ctx.session?.language || 'en';
      const loadingMsg = await ctx.reply(getMessage('pools.fetching', lang));
      
      const pools = await FetchMeteoraPoolService.fetchTopPools();
      ...
      const poolsMessage = pools.map((pool, index) => 
          formatMeteoraPoolMessage(pool, index + 1) 
      ).join('\n\n');

      // 直接发送消息，不包含按钮
      await ctx.reply(`🌟 <b>Top Meteora Pools</b>\n\n${poolsMessage}`, {
          parse_mode: 'HTML',
          disable_web_page_preview: true
      });

    } catch (error) {
      console.error('Error in meteoraTopPools:', error);
      const lang = ctx.session.language || 'en';
...
function formatMeteoraPoolMessage(pool: MeteoraPoolData, index: number): string {
  ...
  const formatPair = () => {
      ...
      // 使用pool.id构建DexScreener链接 (Meteora pools on Solana)
...
export default meteoraTopPoolsCommand;